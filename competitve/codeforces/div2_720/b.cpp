...
bool isPrime(long long N){ // works only for primes < sieve_size*sieve_size(10**14, if sieve_size = 10**7)
    if(N < s){ return p[N]; } // O(1) for small N < sieve_size
...
void solve() 
{
    // find two primes bigger than the smallet element in array;
    int n; cin >> n;
    int minE = 1<<30;
    int minI = -1;
    for(int i=0; i<n; ++i){
    ...
    for(int i =0;i<n; ++i){
        if(i==minI) continue;
        if(i&1){
            cout << minI+1 << " " << i+1 << " " << minE << " " << fp <<endl;
        }else{
            cout << minI+1 << " " << i+1 << " " << minE << " "  << sp << endl;
        }
    }

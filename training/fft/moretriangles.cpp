...
// int up to 2*10^9 (2^31-1) -> (9*(10**4)) ** 2
...
void solve() 
{
    ...
    for(ll i =1ll; i<(ll)n;++i){
       freq[(i*i)%n]++; // freqs for all square numbers mod m
    }
    a =  b = c = freq;
    ret = multiply(a, b); // (a^2 mod m + b^2 mod m)
    ll cnt = 0;


    // as a<=b we can already add all a==a solutions
    for(ll i =1ll; i<(ll)n;++i){
        ret[(2ll*((i*i) %n))]--;  // subtract it from final result-array
        cnt+= freq[(2*((i*i) %n))%n]; // the right side gives all possible c, s.t. a^2 + a^2 = c^2 (mod m)
    }

    // for the rest of the result-array, we need to divide all possibilities i = a^2%m + b^2%m by 2, as one half is a>b
    for(int i =0; i<ret.size(); ++i){
        if(freq[i%n]) // freq[i%n] > 0 only if i is square
            cnt += (ret[i]/2) * freq[i%n];

...
vector<ll> multiply(vector<cd> p1, vector<cd> p2){ // O(n*log(n))
...
vector<ll> stringMatchingMultiply(vector<cd> p1, vector<cd> p2){
    ...
    vector<ll>ret;
    for(cd a: mult){
        ret.push_back((ll)(a.real() + .000000001)); // rounding up only values which are very close to the next element
    }
    ret.resize(p1.size() + p2.size() -1);
    return ret;
}
...
vector<ll> stringMatching(string a, string query){
    // String matching algorithmn. Unlike KMP, which has a slightly better runtime, FFT is more versitile.
